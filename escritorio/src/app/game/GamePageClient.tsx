...
import { ClipboardList, MessageSquare, Undo2, Clock, Footprints, PhoneCall, Bell, ChevronDown, UserPlus, UserMinus, Settings, Share2, LogOut, Pencil, Users, Globe, RotateCcw, Info, Building2, BarChart3, BookOpen, FolderOpen, Server, Wrench } from "lucide-react";
...
import LibraryStorybook from "@/components/library/LibraryStorybook";
import ChannelOpsLibraryModal from "@/components/ChannelOpsLibraryModal";
import NewClientWizard from "@/components/NewClientWizard";
import OfficeRoomsMenu from "@/components/OfficeRoomsMenu";
import StorybookModal from "@/components/StorybookModal";
import UsageReportModal from "@/components/UsageReportModal";
import NpcDockPanel from "@/components/NpcDockPanel";
import NpcCommandCenter from "@/components/NpcCommandCenter";
import type { Task } from "@/components/TaskCard";
import type { NpcPowersState } from "@/components/NpcPowersFields";
...
interface ChannelNpcSummary {
  ...
  hasAgent?: boolean;
  agentId?: string | null;
  runtimeProvider?: "channel-default" | "openclaw" | "claude-code" | "codex" | null;
  model?: string | null;
...
function GamePageInner() {
  ...
  const [managedNpcId, setManagedNpcId] = useState<string | null>(null);

  // Owner & NPC management state
  const [isOwner, setIsOwner] = useState(false);
  const [showHireModal, setShowHireModal] = useState(false);
  const [showNewClientWizard, setShowNewClientWizard] = useState(false);
  const [libraryViewerTarget, setLibraryViewerTarget] = useState<{ channelId: string; name: string } | null>(null);
  const [opsLibraryTarget, setOpsLibraryTarget] = useState<{ channelId: string; name: string } | null>(null);
  const [placementMode, setPlacementMode] = useState(false);
  const [spawnSetMode, setSpawnSetMode] = useState(false);
  const [pendingNpc, setPendingNpc] = useState<{ presetId?: string; name: string; persona: string; appearance: unknown; direction: string; agentId?: string; agentAction?: "select" | "create"; identity?: string; soul?: string; locale?: string; powers?: NpcPowersState; runtimeProvider?: "channel-default" | "openclaw" | "claude-code" | "codex"; model?: string } | null>(null);
  const [relocatingNpc, setRelocatingNpc] = useState<{ id: string; name: string; direction?: string } | null>(null);
  const [editingNpc, setEditingNpc] = useState<{ id: string; name: string; persona: string; appearance: unknown; direction?: string; agentId?: string | null; runtimeProvider?: "channel-default" | "openclaw" | "claude-code" | "codex" | null; model?: string | null; identity?: string | null; soul?: string | null; powers?: NpcPowersState | null; passPolicy?: string | null; presetId?: string | null } | null>(null);
  ...
  const openChannelMapEditor = useCallback(() => {
    if (!channelId) return;
    setShowChannelSettings(false);
    setShowUserMenu(false);
    const returnTo = `/game?channelId=${channelId}&characterId=${characterId}`;
    const params = new URLSearchParams({ returnTo });
    router.push(`/map-editor/channel/${channelId}?${params.toString()}`);
  ...
  const showToastNotification = useCallback((id: string, message: string) => {
    ...
    setNotifications((prev) =>
      [{ id, message, timestamp: Date.now(), read: false }, ...prev.filter((n) => n.id !== id)].slice(0, 100),
    );
  }, []);
  ...
  const refreshChannelNpcs = useCallback(async () => {
    if (!channelId) return;
    const response = await fetch(`/api/npcs?channelId=${encodeURIComponent(channelId)}`);
  ...
  useEffect(() => {
    ...
    import("socket.io-client").then(({ io }) => {
      ...
      socketInstance.on("npc:history", (data: { npcId: string; messages: { role: string; content: string; timestamp?: number; provider?: string; model?: string; sender?: string }[] }) => {
      ...
      socketInstance.on("npc:history-append", (data: { npcId: string; message?: string; content?: string; role?: string; timestamp?: number; provider?: string; model?: string; sender?: string }) => {
        const role = data.role === "player" ? "player" : "npc";
        const rawContent = data.content ?? data.message ?? "";
        const cleaned = role === "npc" ? sanitizeNpcResponseText(rawContent) : String(rawContent).trim();
        if (!cleaned.trim()) return;
        if (dialogNpcRef.current?.npcId !== data.npcId) return;

        setNpcMessages((prev) => {
          if (role === "npc" && prev.some((message) => message.role === "npc" && message.content === cleaned)) {
            return prev;
          }
          return [...prev, { role, content: cleaned, timestamp: data.timestamp, provider: data.provider, model: data.model, sender: data.sender }];
        });
      });
      ...
      socketInstance.on("chat:message", (msg: ChannelChatMessage) => {
        ...
        if (msg.senderCharacterId !== characterId) {
        ...
        if (chunk) {
          ...
          setNpcMessages((prev) => {
            ...
            if (last && last.role === "npc") {
              const updated = [...prev];
              updated[updated.length - 1] = { role: "npc", content: buffered, timestamp: Date.now(), provider: data.provider, model: data.model };
              return updated;
            }
            return [...prev, { role: "npc", content: buffered, timestamp: Date.now(), provider: data.provider, model: data.model }];
          });
        }
        if (data.done) {
          setIsNpcStreaming(false);
          ...
          setNpcMessages((prev) => {
            ...
            if (lastIdx >= 0 && prev[lastIdx].role === "npc") {
              const updated = [...prev];
              updated[lastIdx] = { role: "npc", content: cleaned, timestamp: Date.now(), provider: data.provider, model: data.model };
              return updated;
            }
            return prev;
          });
  ...
  const handleDialogSend = useCallback(
    async (message: string, files?: File[]) => {
      ...
      const displayMessage = files && files.length > 0
        ...
        : message;
      setNpcMessages((prev) => [...prev, { role: "player", content: displayMessage, timestamp: Date.now(), sender: character?.name }]);
      streamBufferRef.current = "";
      ...
      (async () => {
        try {
          ...
          if (!channelRes.ok) {
            ...
            throw new Error(getLocalizedErrorMessage(t, data, "errors.failedToLoadGameData"));
          }
          ...
          const nextChannel = channelData.channel as ChannelInfo;
          setChannel(nextChannel);
          setIsOwner(true);
        } catch (err) {
          if (!ignore) {
            setError(err instanceof Error ? err.message : t("errors.failedToLoadGameData"));
          }
        } finally {
  ...
  useEffect(() => {
    const onReady = () => { sceneReadyRef.current = true; };
    EventBus.on("scene-ready", onReady);
    return () => { EventBus.off("scene-ready", onReady); sceneReadyRef.current = false; };
  }, []);

  // Synchronize NPCs with Phaser whenever they change in React.
  // Emit immediately (works if scene already ready) AND re-emit on scene-ready
  // (covers the race where scene-ready fired before our listener mounted).
  useEffect(() => {
    if (!channelNpcs || channelNpcs.length === 0) return;
    EventBus.emit("npc:sync-all", channelNpcs);
    const onReady = () => { EventBus.emit("npc:sync-all", channelNpcs); };
    EventBus.on("scene-ready", onReady);
    return () => { EventBus.off("scene-ready", onReady); };
  }, [channelNpcs]);

  const handleSaveNpcEdit = async (
    npcId: string,
    updates: (Record<string, unknown> & { powers?: NpcPowersState | null }) | null | undefined,
  ) => {
    try {
      const { powers, ...patchUpdates } = updates || {};
      ...
      if (powers) {
        try {
          const { powersStateToPayload } = await import("@/components/NpcPowersFields");
          await fetch(`/api/npcs/${npcId}/powers`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(powersStateToPayload(powers)),
          });
        } catch (err) {
          console.error("Failed to save NPC powers:", err);
        }
      }
      await refreshChannelNpcs();
  ...
  useEffect(() => {
    if (placementMode && (pendingNpc || relocatingNpc)) {
      EventBus.emit("placement-mode-start", pendingNpc ?? relocatingNpc);
    }
    const onPlacementComplete = async (data: { col: number; row: number }) => {
      if (relocatingNpc) {
        let keepPlacementMode = false;
        try {
          const res = await fetch(`/api/npcs/${relocatingNpc.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              positionX: data.col,
              positionY: data.row,
              direction: relocatingNpc.direction,
            }),
          });
          if (res.status === 409) {
          ...
          if (!res.ok) {
            const errorData = await res.json().catch(() => ({}));
            throw new Error(getLocalizedErrorMessage(t, errorData, "errors.failedToUpdateNpc"));
          }

          const result = await res.json();
          await refreshChannelNpcs();
          const localUpdate = {
            npcId: relocatingNpc.id,
            positionX: data.col,
            positionY: data.row,
            direction: result.npc?.direction || relocatingNpc.direction,
          };
          EventBus.emit("npc:update-local", localUpdate);
          if (socket) socket.emit("npc:broadcast-update", localUpdate);
        } catch (err) {
          keepPlacementMode = true;
          console.error("Failed to move NPC:", err);
          showToastNotification(
            "npc-move-error",
            err instanceof Error ? err.message : t("errors.failedToUpdateNpc"),
          );
        } finally {
          if (keepPlacementMode) return;
          setPlacementMode(false);
          setRelocatingNpc(null);
      ...
      try {
        const res = await fetch("/api/npcs", {
          ...
          body: JSON.stringify({
            ...
            runtimeProvider: pendingNpc.runtimeProvider,
            model: pendingNpc.model,
          }),
        });
        if (res.status === 409) {
          keepPlacementMode = true;
        ...
        if (res.ok) {
          const result = await res.json();
          if (pendingNpc.powers && result.npc?.id) {
            try {
              const { powersStateToPayload } = await import("@/components/NpcPowersFields");
              await fetch(`/api/npcs/${result.npc.id}/powers`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(powersStateToPayload(pendingNpc.powers)),
              });
            } catch (err) {
              console.error("Failed to save NPC powers:", err);
            }
  ...
  }, [placementMode, pendingNpc, relocatingNpc, channelId, refreshChannelNpcs, showToastNotification, socket, t]);
  ...
  useEffect(() => {
    const onNpcEdit = async (data: { npcId: string }) => {
      if (!channelId) return;
      // Sempre buscar dados frescos da API para garantir identity/soul/powers atualizados
      let raw: Record<string, unknown> | null = null;
      try {
        const res = await fetch(`/api/npcs?channelId=${encodeURIComponent(channelId)}`);
        if (res.ok) {
          const j = await res.json();
          raw = (j.npcs || []).find((n: { id: string }) => n.id === data.npcId) as Record<string, unknown> | null;
        }
      } catch { /* cai no fallback */ }
      if (!raw) {
        const fallback = channelNpcs.find(n => n.id === data.npcId);
        if (!fallback) return;
        raw = fallback as unknown as Record<string, unknown>;
      }
      setEditingNpc({
        id: raw.id as string, name: raw.name as string,
        persona: (raw.persona as string) || "",
        appearance: raw.appearance,
        direction: typeof raw.direction === "string" ? raw.direction as string : "down",
        agentId: (raw.agentId as string | null) || null,
        runtimeProvider: (raw.runtimeProvider as "channel-default" | "openclaw" | "claude-code" | "codex" | null) || "channel-default",
        model: (raw.model as string | null) || null,
        identity: (raw.identity as string | null) || null,
        soul: (raw.soul as string | null) || null,
        powers: (raw.powers as NpcPowersState | null | undefined) ?? null,
        passPolicy: (raw.passPolicy as string | null) || null,
  ...
  }, [channelNpcs, channelId, refreshChannelNpcs, showToastNotification, socket, t]);
  ...
  const handleOpenCommandCenterById = useCallback((npcId: string) => {
    setManagedNpcId(npcId);
    setContextMenu(null);
    closeRosterMenus();
  }, [closeRosterMenus]);
  ...
  const handleRelocateNpc = useCallback(() => {
    if (!contextMenu) return;
    const npc = channelNpcs.find((entry) => entry.id === contextMenu.npcId);
    setRelocatingNpc({
      id: contextMenu.npcId,
      name: contextMenu.npcName,
      direction: npc?.direction || "down",
    });
    setPlacementMode(true);
    setContextMenu(null);
    closeRosterMenus();
  }, [channelNpcs, closeRosterMenus, contextMenu]);
    ...
    return (
      <div className="min-h-screen w-screen overflow-y-auto bg-[#070d1c] text-white">
        {loading && (
          <div className="min-h-screen flex items-center justify-center text-white">
            <div className="text-center">
              ...
              <div className="text-gray-400">Carregando Command Center do NPC...</div>
            </div>
          </div>
        )}
        ...
        {!loading && !error && (!channelId || !managedNpc) && (
          <div className="min-h-screen flex items-center justify-center text-white">
            <div className="text-center">
              <div className="text-xl mb-2">Command Center indisponível</div>
              <div className="text-gray-400">Não encontrei este NPC no canal do Escritório Virtual.</div>
            </div>
          </div>
        )}
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-gray-900 text-white">
          ...
          <PhaserGame
            ...
            containerClassName={
              mode === "office" && channelId
                ? "fixed top-0 bottom-0 left-0 right-0 lg:right-[260px] z-0"
                : "fixed inset-0 z-0"
            }
          />
        )}
      ...
      {mode === "office" && (
        <>
      {/* Top bar */}
      <div className="fixed top-0 left-0 right-0 lg:right-[260px] z-40 flex h-14 items-center gap-3 overflow-visible px-3 sm:px-4 py-2 bg-black/75 backdrop-blur-sm border-b border-white/10">
        {/* Left: Channel name — Character name */}
        <h1 className="min-w-[120px] max-w-[34vw] shrink truncate text-sm sm:text-lg font-bold">
          {channel?.name || "DeskRPG"} &mdash; {character?.name}
        </h1>

        {/* Right: grouped controls */}
        <div className="min-w-0 flex flex-1 justify-end">
          <div className="flex flex-nowrap items-center justify-end gap-1.5 pl-2">
          {/* Gateway status */}
          {channel?.hasGateway ? (
            <button
              onClick={() => openChannelSettings("gateway")}
              className="shrink-0 flex items-center gap-1.5 px-2 py-1 rounded-md bg-sky-500/10 border border-sky-400/20 text-caption text-sky-200 hover:bg-sky-500/20"
            >
              <span className="w-2 h-2 rounded-full bg-sky-300" />
              <span className="hidden md:inline">{t("game.aiGateway")}</span>
              <span className="md:hidden">AI</span>
            </button>
          ) : (
            <button
              onClick={() => openChannelSettings("gateway")}
              className="shrink-0 flex items-center gap-1.5 px-2 py-1 rounded-md bg-amber-500/10 border border-amber-400/20 text-caption text-amber-200 hover:bg-amber-500/20"
            >
              <span className="w-2 h-2 rounded-full bg-amber-300" />
              <span className="hidden md:inline">{t("game.gatewayConnect")}</span>
              <span className="md:hidden">AI</span>
            </button>
          )}
              ...
              <button
                onClick={() => {
	              ...
	              >
	                ...
	                <span className="hidden xl:inline">{t("game.playersOnlineCount", { count: channelPlayers.length })}</span>
	                <span className="xl:hidden">{channelPlayers.length}</span>
	              </button>
              <button
                onClick={() => {
                  setRosterActionMenu(null);
	              ...
	              >
	                ...
	                <span className="hidden xl:inline">{t("game.npcsAtWorkCount", { count: channelNpcs.length })}</span>
	                <span className="xl:hidden">{channelNpcs.length}</span>
	              </button>
            </div>

            {showRosterMenu && (
              <div className="absolute top-full left-0 mt-2 w-64 bg-surface border border-border rounded-lg shadow-xl z-[70] overflow-hidden">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-md text-caption font-semibold bg-meeting/80 hover:bg-meeting text-white"
		          >
		            <Users className="w-3 h-3" />
		            <span className="hidden lg:inline">{t("game.meetingRoom")}</span>
          ...
          <button
            onClick={() => setShowTaskBoard(true)}
	            className="shrink-0 flex items-center gap-1 px-2.5 py-1 bg-primary/80 hover:bg-primary text-white rounded-md text-caption font-semibold"
	          >
	            <ClipboardList className="w-3 h-3" /> <span className="hidden sm:inline">{t("game.tasks")}</span>
            {(() => {
          ...
          <div className="relative">
            <button
              onClick={() => {
                ...
                closeRosterMenus();
	              ...
	              className="shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-caption text-text-secondary hover:text-white hover:bg-white/10"
            >
            ...
            {showToolsMenu && (
              <div className="absolute right-0 top-full mt-2 w-60 bg-surface border border-border rounded-lg shadow-xl z-[70] py-1">
                <button
                  onClick={() => { setShowUsageReport(true); setShowToolsMenu(false); }}
                ...
                <button
                  onClick={() => { setShowStorybook(true); setShowToolsMenu(false); }}
                  className="w-full text-left px-4 py-2 text-body text-text-secondary hover:bg-surface-raised hover:text-white flex items-center gap-2"
                >
                ...
                {channelId && channel && (
                  <button
                    onClick={() => { setShowLibraryPanel((value) => !value); setShowToolsMenu(false); }}
                    className="w-full text-left px-4 py-2 text-body text-text-secondary hover:bg-surface-raised hover:text-white flex items-center gap-2"
                  >
                ...
                {channelId && channel && (
                  <button
                    onClick={() => { setOpsLibraryTarget({ channelId, name: channel.name || "Operações" }); setShowToolsMenu(false); }}
                    className="w-full text-left px-4 py-2 text-body text-text-secondary hover:bg-surface-raised hover:text-white flex items-center gap-2"
                  >
                ...
                <button
                  onClick={() => { setShowOrgChart(true); setShowToolsMenu(false); }}
                  className="w-full text-left px-4 py-2 text-body text-text-secondary hover:bg-surface-raised hover:text-white flex items-center gap-2"
                >
                  <Users className="w-3.5 h-3.5" />
                  Organograma
                </button>
                {channel?.channelType === "hq" && (
                  <button
                    onClick={() => { setShowNewClientWizard(true); setShowToolsMenu(false); }}
                    className="w-full text-left px-4 py-2 text-body text-amber-200 hover:bg-surface-raised hover:text-amber-100 flex items-center gap-2"
                  >
          ...
          <div className="relative">
            <button
              onClick={() => { setShowUserMenu(!showUserMenu); setShowSharePopup(false); setShowToolsMenu(false); closeRosterMenus(); }}
	              className="shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-caption text-text-secondary hover:text-white hover:bg-white/10 relative"
            >
              <Settings className="w-3.5 h-3.5" />
	              <span className="hidden sm:inline">{t("game.menuSettings")}</span>
            ...
            {showUserMenu && (
              <div className="absolute right-0 top-full mt-2 bg-surface border border-border rounded-lg shadow-xl w-56 z-[70] py-1">
                {isOwner && (
                  <button
                    onClick={openChannelMapEditor}
                    className="w-full text-left px-4 py-2 text-body text-text-secondary hover:bg-surface-raised hover:text-white flex items-center gap-2"
                  >
          ...
          {showSharePopup && channel?.inviteCode && (
            <div className="fixed top-16 right-4 bg-surface border border-border rounded-lg p-3 shadow-xl w-72 z-[70]">
      ...
      {(showUserMenu || showToolsMenu || showRosterMenu || showSharePopup) && (
        <div
      ...
      {showNotificationsModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl max-h-[82vh] rounded-xl border border-border bg-surface shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div>
                <h2 className="text-lg font-semibold text-text">Todas as notificações</h2>
                <p className="text-caption text-text-dim">
                  {notifications.filter((n) => !n.read).length} pendente(s) de {notifications.length}
                </p>
              </div>
              <div className="flex items-center gap-2">
                {notifications.length > 0 && (
                  <button
                    onClick={() => setNotifications((prev) => prev.map((n) => ({ ...n, read: true })))}
                    className="px-3 py-1.5 rounded-md border border-border text-caption text-text-secondary hover:bg-surface-raised hover:text-text"
                  >
                    {t("game.markAllRead")}
                  </button>
                )}
                <button
                  ...
                  className="w-8 h-8 rounded-md border border-border text-text-secondary hover:bg-surface-raised hover:text-text"
            ...
            <div className="overflow-y-auto p-4 space-y-2">
              {notifications.length === 0 ? (
                <div className="py-10 text-center text-text-dim">{t("game.noNotifications")}</div>
              ) : (
                notifications.map((n) => (
                  <div
                    key={n.id}
                    className={`rounded-lg border px-4 py-3 ${
                      n.read
                        ...
                        : "border-primary/30 bg-primary/10 text-text-secondary"
                    }`}
                  >
          ...
          <div className="w-full max-w-2xl rounded-xl border border-border bg-surface shadow-2xl">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-[180px_1fr] gap-x-4 gap-y-4 text-sm">
                <div className="text-text-dim">{t("about.product")}</div>
                <div className="text-text">{OFFICE_PRODUCT_NAME}</div>

                <div className="text-text-dim">{t("about.version")}</div>
                <div className="text-text">v{APP_VERSION}</div>

                <div className="text-text-dim">{t("about.role")}</div>
                <div className="text-text-secondary">{t("about.roleDescription")}</div>

                <div className="text-text-dim">{t("about.aiUnlock")}</div>
                <div className="text-text-secondary">{t("about.aiUnlockDescription")}</div>
              ...
              <details className="rounded-lg border border-border bg-surface-raised/40 px-4 py-3 text-sm">
                <summary className="cursor-pointer text-text-secondary hover:text-text">
                  {t("about.legalDetails")}
                </summary>
                <div className="mt-4 grid grid-cols-[180px_1fr] gap-x-4 gap-y-3">
      ...
      {channelId && (
        <OrgChartModal
          ...
          onHireNew={() => { setShowOrgChart(false); setEditingNpc(null); setShowHireModal(true); }}
        />
      )}
      ...
      {showChannelSettings && channel && (
        <ChannelSettingsModal
          ...
          onEditMap={openChannelMapEditor}
            ...
            setChannel((prev) => {
              ...
              return {
                ...
                hasGateway:
                  data.gatewayConfig
                    ? Boolean(
                        data.gatewayConfig.provider === "claude-code"
                        || data.gatewayConfig.provider === "codex"
      ...
      {/* Library panel — shown in HQ and client channels, floating only in the office */}
      {mode === "office" && (channel?.channelType === "hq" || channel?.channelType === "client") && channelId && (
        <LibraryPanel
          channelId={channelId}
          channelName={channel?.name || ""}
          ...
          onOpenLibrary={(targetId, targetName) => {
            setShowLibraryPanel(false);
            setLibraryViewerTarget({ channelId: targetId, name: targetName });
          }}
        />
      )}
      ...
      {mode === "office" && channelId && !hideEmbeddedNpcDock && (
        <NpcDockPanel
          ...
          onEditNpc={(npcId) => EventBus.emit("npc:edit", { npcId })}
          onHireNpc={isOwner ? () => setShowHireModal(true) : undefined}
        />
      )}

      {/* Library viewer popup */}
      {libraryViewerTarget && (
        <LibraryStorybook
          channelId={libraryViewerTarget.channelId}
          channelName={libraryViewerTarget.name}
          visibleLayers={["design", "documents", "social", "creations"]}
          onClose={() => setLibraryViewerTarget(null)}
        />
      )}

      {opsLibraryTarget && (
        <ChannelOpsLibraryModal
          channelId={opsLibraryTarget.channelId}
          channelName={opsLibraryTarget.name}
          onClose={() => setOpsLibraryTarget(null)}
        />
      )}

      {/* New Client Wizard */}
      {showNewClientWizard && channel && channelId && characterId && (
        <NewClientWizard
      ...
      {/* Back to HQ button — shown in client channels */}
      {channel?.channelType === "client" && channel?.parentChannelId && characterId && (
        <Link
          href={`/game?channelId=${channel.parentChannelId}&characterId=${characterId}`}
          className="fixed top-16 right-4 z-40 bg-gray-900/90 border border-gray-700 hover:border-amber-500 text-amber-400 text-xs font-medium px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors"
        >
      ...
      {placementMode && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 bg-primary text-white px-4 py-2 rounded-lg shadow-lg text-body font-medium">
          {relocatingNpc ? t("game.relocateNpcMode", { name: relocatingNpc.name }) : t("game.placementMode")}
        </div>
      )}
        ...
        return <>
          ...
          >
            <div className="bg-surface border border-border rounded-lg shadow-xl py-1 min-w-[140px]">
              ...
              {isOwner && currentMoveState === "idle" && (
                <button
                  onClick={handleRelocateNpc}
                  className="w-full text-left px-3 py-2 text-body text-text hover:bg-surface-raised"
                >
                  <Pencil className="w-3.5 h-3.5 inline mr-1" />{t("npc.moveOnMap")}
                </button>
              )}
              {isOwner && (
                <button
                  onClick={() => handleOpenCommandCenterById(contextMenu.npcId)}
                  className="w-full text-left px-3 py-2 text-body text-text hover:bg-surface-raised"
                >
                  <Settings className="w-3.5 h-3.5 inline mr-1" />Gerenciar
                </button>
              )}
              {isOwner && (
                <button
          ...
          <>
            ...
            >
              <div className="bg-surface border border-border rounded-lg shadow-xl py-1 min-w-[160px]">
                ...
                {isOwner && (
                  <button
                    onClick={() => handleOpenCommandCenterById(rosterActionMenu.npcId)}
                    className="w-full text-left px-3 py-2 text-body text-text hover:bg-surface-raised"
                  >
                    <Settings className="w-3.5 h-3.5 inline mr-1" />Gerenciar
                  </button>
      ...
      <NpcHireModal
        ...
        onClose={() => {
          setShowHireModal(false);
          setEditingNpc(null);
          EventBus.emit("dialog:active", false); // Resume AI nudges
        }}
        onPlaceOnMap={(npcData) => {
          setPendingNpc(npcData);
          setShowHireModal(false);
      ...
      {mode === "meeting" && character && (
        <MeetingRoom
          ...
          libraryPanel={(channel?.channelType === "hq" || channel?.channelType === "client") && channelId ? (
            <LibraryPanel
              channelId={channelId}
              channelName={channel?.name || ""}
              channelType={channel?.channelType ?? null}
